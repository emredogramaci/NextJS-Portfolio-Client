...
import usePersonelInfo from "hooks/usePersonalInfo";

const Contact = () => {
  const [isOpen, setIsOpen] = useState(false);
  const { personalInfo } = usePersonelInfo();

  return (
    <BannerLayout>
      <div className=" px-4 py-2">
        <div className="my-6 text-Snow flex flex-col gap-y-5">
          <h1 className="text-lg font-bold">İletişim Bilgileri</h1>
          <div className="flex flex-col md:flex-row items-center gap-5 text-xs">
            <div className="card_stylings w-full md:w-1/2 p-5 md:p-6 lg:p-8 flex flex-col gap-y-4">
              <div className="flex justify-between items-center">
                <span className="md:text-base">Ülke:</span>
                <span className="text-LightGray md:text-sm">
                  {personalInfo.Country}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="md:text-base">Şehir:</span>
                <span className="text-LightGray md:text-sm">
                  {personalInfo.City}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="md:text-base">Şirket:</span>
                <span className="text-LightGray md:text-sm">
                  {personalInfo.Corp}
                </span>
              </div>
            ...
            <div className="card_stylings rounded-xl w-full md:w-1/2 p-5 md:p-6 lg:p-8 flex flex-col gap-y-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-LightGray text-sm">emredogramaci</span>
              </div>
              <div className="flex justify-between items-center">
